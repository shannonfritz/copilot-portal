									...
									>
										...
										{confirmDeleteInstruction === inst.id ? (
											...
											<span className="flex gap-0.5 shrink-0" onClick={e => e.stopPropagation()}>
												...
												<button className="rounded p-1.5" style={{ opacity: inst.hasPrompts ? 0.7 : 0.2 }} onClick={async (e) => {
													e.stopPropagation();
						...
						<div className="flex-1 rounded-xl border" style={{ borderColor: 'var(--border)', background: 'var(--bg)' }}>
							{showPromptsTray && sessionPrompts.length > 0 && (
								...
								{sessionPrompts.length > 5 && (
									<div className="pointer-events-none absolute bottom-0 left-0 right-0 z-10" style={{ height: 48, background: 'linear-gradient(transparent 0%, var(--bg) 70%)' }} />
								)}
								</div>
							)}
							<div className="relative">
