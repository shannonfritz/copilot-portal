...
- **Agent tooling bundled** — `git`, `gh`, `wget`, `zip`/`unzip`, `patch`, `make`, `jq` (plus `openssh-client`, `less`) are baked into the image so the agent can clone/commit/diff, use the GitHub CLI, and handle archives out of the box; tools are added at image-build time since the non-root runtime user can't `apt install` at runtime. Kept lean — language runtimes/compilers are omitted by default
